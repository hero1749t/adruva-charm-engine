...
const MenuItemCard = ({ item, onToggle, onEdit, onDelete }: MenuItemCardProps) => (
  <div className={`bg-card rounded-xl border border-border p-3 shadow-card ${!item.is_available ? "opacity-60" : ""}`}>
    <div className="flex gap-3">
      {item.image_url && (
        <img src={item.image_url} alt={item.name} className="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-lg flex-shrink-0" />
      )}
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <span className={`w-2.5 h-2.5 rounded-sm flex-shrink-0 ${item.is_veg ? "bg-green-600" : "bg-red-600"}`} />
